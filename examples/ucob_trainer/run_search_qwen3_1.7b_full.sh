...
export RAY_TMPDIR=/tmp/ray_ucob_search
export TMPDIR=/tmp
export HF_HOME=/tmp/hf_ucob_search
export HF_DATASETS_CACHE=/tmp/hf_ucob_search/datasets

MODEL_PATH="${MODEL_PATH:-Qwen/Qwen3-1.7B}"

TRAIN_PARQUET="${SEARCH_DATA_DIR:-${HOME}/data/searchR1_processed_direct}/train.parquet"
VAL_PARQUET="${SEARCH_DATA_DIR:-${HOME}/data/searchR1_processed_direct}/test.parquet"
SEARCH_RETRIEVER_HOST=127.0.0.1
SEARCH_RETRIEVER_PORT=8000
SEARCH_RETRIEVER_URL=http://0.0.0.0:8000/retrieve
SEARCH_RETRIEVER_LOG_DIR=outputs/logs/search_retriever

search_retriever_ready() {
    python3 - "$SEARCH_RETRIEVER_HOST" "$SEARCH_RETRIEVER_PORT" <<'PY'
import socket
import sys
...
if ! search_retriever_ready; then
    mkdir -p "$SEARCH_RETRIEVER_LOG_DIR"
    retriever_log="${SEARCH_RETRIEVER_LOG_DIR}/search_retriever_$(date +%Y%m%d_%H%M%S).log"
    echo "Search retriever is not running on ${SEARCH_RETRIEVER_HOST}:${SEARCH_RETRIEVER_PORT}; starting it in background."
    bash examples/search/retriever/retrieval_launch.sh >"$retriever_log" 2>&1 &
    retriever_pid=$!
    echo "Search retriever pid=${retriever_pid}, log=${retriever_log}"
    for _ in $(seq 1 120); do
        if search_retriever_ready; then
            break
        fi
        if ! kill -0 "$retriever_pid" 2>/dev/null; then
            echo "Search retriever exited before becoming ready. Last log lines:" >&2
            tail -80 "$retriever_log" >&2
            exit 1
        fi
        sleep 5
    done
...
if ! search_retriever_ready; then
    echo "Search retriever did not become ready on ${SEARCH_RETRIEVER_HOST}:${SEARCH_RETRIEVER_PORT}." >&2
    exit 1
fi
...
experiment_name="ucob_full_search_qwen3_1.7b"
skill_memory_file="outputs/dynamic_memory/${experiment_name}/search_reflections.json"
...
python3 -m verl.trainer.main_ucob \
    ...
    algorithm.ucob.skill_memory.skill_mapping_dir=skills/search \
    ...
    algorithm.ucob.skill_memory.max_trajectory_steps=4 \
    algorithm.ucob.skill_memory.max_reflection_items=4 \
    ...
    env.search.search_url=$SEARCH_RETRIEVER_URL \
    env.search.eval_metric_mode=trajectory \
    env.resources_per_worker.num_cpus=$num_cpus_per_env_worker \
    trainer.critic_warmup=0 \
    trainer.logger=['console','swanlab'] \
    trainer.project_name='UCOB_search' \
