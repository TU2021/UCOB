...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        meta_sampling_kwargs = {}
        if "response_length" in prompts.meta_info:
            meta_sampling_kwargs["max_tokens"] = int(prompts.meta_info["response_length"])
        if "max_tokens" in prompts.meta_info:
            meta_sampling_kwargs["max_tokens"] = int(prompts.meta_info["max_tokens"])
        for key in ("temperature", "top_p", "top_k"):
            if key in prompts.meta_info:
                meta_sampling_kwargs[key] = prompts.meta_info[key]
        if meta_sampling_kwargs:
            kwargs.update(meta_sampling_kwargs)
        ...
        with self.update_sampling_params(**kwargs):
            ...
            response = [list(response_ids)[:target_response_length] for response_ids in response]
            rollout_log_probs = [list(log_probs)[:target_response_length] for log_probs in rollout_log_probs]
