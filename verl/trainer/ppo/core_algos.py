...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    if loss_agg_mode == "token-mean":
        ...
        if active_rows.any():
            loss = torch.sum(seq_losses * active_rows.to(dtype=seq_losses.dtype)) / active_rows.sum().clamp(min=1)
        else:
            loss = seq_losses.sum() * 0.0
...
def compute_policy_loss(
    ...
    negative_approx_kl = torch.where(valid_mask, negative_approx_kl, torch.zeros_like(negative_approx_kl))
    negative_approx_kl = torch.nan_to_num(negative_approx_kl, nan=0.0, posinf=10.0, neginf=-10.0)
...
def compute_policy_loss_gspo(
    ...
    safe_log_prob = torch.where(valid_mask, log_prob, log_prob.detach())
    log_seq_importance_ratio = safe_log_prob - safe_log_prob.detach() + negative_approx_kl_seq.detach().unsqueeze(-1)
    log_seq_importance_ratio = torch.where(valid_mask, log_seq_importance_ratio, torch.zeros_like(log_seq_importance_ratio))
    log_seq_importance_ratio = torch.nan_to_num(log_seq_importance_ratio, nan=0.0, posinf=10.0, neginf=-10.0)
