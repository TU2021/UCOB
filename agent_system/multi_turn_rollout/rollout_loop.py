...
class TrajectoryCollector:
    ...
    def gather_rollout_data(
            ...
            discounted_returns: np.ndarray = None,
            ) -> DataProto:
        """
        ...
        Parameters:
            ...
            episode_rewards (np.ndarray): Per-turn rewards for each environment
            discounted_returns (np.ndarray): Per-turn discounted returns for each environment
        ...
        if discounted_returns is None or not isinstance(episode_rewards[0], (list, tuple, np.ndarray)):
            episode_rewards, discounted_returns = self.no_credit_assignment(total_batch_list, episode_rewards)
        ...
        for bs in range(batch_size):
            ep_rew_arr = episode_rewards[bs]
            disc_ret_arr = discounted_returns[bs]
            for step_idx, data in enumerate(total_batch_list[bs]):
                assert traj_uid[bs] == data['traj_uid'], "data is not from the same trajectory"
                if data['active_masks']:
                    data['episode_rewards'] = np.array(ep_rew_arr[step_idx], dtype=np.float32)
                    data['step_returns'] = torch.tensor(float(disc_ret_arr[step_idx]), dtype=torch.float32)
                    # episode_lengths
    ...
    def credit_assignment(self, total_batch_list, step_gamma=0.95):
        """
        Compute per-turn outcome rewards and backward-discounted returns.

        episode_rewards keeps the cumulative trajectory reward on each active
        turn. discounted_returns applies step_gamma over the actual per-turn
        environment rewards, matching Skill1/RetroAgent-style credit assignment.
        """
    ...
    def no_credit_assignment(self, total_batch_list, final_episode_rewards):
        """
        Preserve the previous behavior: copy each trajectory's final outcome
        reward to every active turn.
        """
        num_trajectories = len(total_batch_list)
