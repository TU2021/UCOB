...
class EnvironmentManagerBase:
    ...
    def require_thinking(self) -> bool:
        env_cfg = getattr(self.config, "env", None)
        if env_cfg is None:
            return False
        return _as_bool(env_cfg.get("require_thinking", False))

    def thinking_instruction(self, prompt_type: str) -> str:
        if prompt_type == "webshop":
            return (
                "You should first reason step-by-step about the current situation, then think carefully "
                "which admissible action best advances the shopping goal. This reasoning process MUST be "
                "enclosed within <think> </think> tags."
            )
        if prompt_type == "sokoban":
            return (
                "You should first reason step-by-step about the current situation: observe the positions "
                "of boxes and targets, plan a path to push a box toward a target, and avoid traps like "
                "corners or walls. This reasoning process MUST be enclosed within <think> </think> tags."
            )
        if prompt_type == "search":
            return "You should first conduct reasoning process. This process MUST be enclosed within <think> </think> tags."
        if prompt_type == "appworld":
            return (
                "You should first reason step-by-step about which APIs to call, what arguments to use, "
                "and how to build your code block to complete the task. This reasoning process MUST be "
                "enclosed within <think> </think> tags."
            )
        if prompt_type == "appworld_history":
            return (
                "First, you MUST carefully reflect on the history of interactions and the most recent "
                "error messages. Then, reason about what should be done next, which APIs to call, what "
                "arguments to use, and how to build your code block to complete the task. This reasoning "
                "process MUST be enclosed within <think> </think> tags."
            )
        return (
            "You should first reason step-by-step about the current situation. This rasoning process "
